...
class DataValidation:
    ...
    def drop_missing_values_columns(self,df:pd.DataFrame,report_key_name)->Optional[pd.DataFrame]:
        """
        ...
        ============================================================================================
        returns pandas dataframe if a atleast a single column is available after missing columns drop 
        """
        try:
            threshold = self.data_validation_config.missing_threshold
            null_report = df.isna().sum()/df.shape[1]
            #selecting column name which conetains null
            logging.info(f"selecting column name which contains null above to {threshold}")
            drop_column_names = null_report[null_report>threshold].index

            logging.info(f" Columns to drop:{list(drop_column_names)} ")
            self.validation_error["droped_columns"] = list(drop_column_names)
    ...
    def initiate_data_validation(self)->artifact_entity.DataValidationArtifact:
        try:
            ...
            base_df = self.drop_missing_values_columns(df=base_df,report_key_name="report_key_name=missing_values_within_base_dataset")
            ...
            if train_df_columns_status:
                logging.info(f"As all columns exists in train df hence detecting data drift ")
                self.data_drift(base_df=base_df,current_df=train_df,report_key_name="data_drift_within_train_dataset")

            if test_df_columns_status:
                logging.info(f"As all columns exists in test df hence detecting data drift ")
